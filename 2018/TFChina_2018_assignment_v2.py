...
for p in list(df_prov_ratio[str_colname_prov_name]): # loop through each province
	...
	## select candidates by preference
	df_cand_prov = df_cand_prov[df_cand_prov[str_colname_prov_preference] == p]
...
writer_remaining = pd.ExcelWriter('remaining.xlsx')
df_remaining.to_excel(writer_remaining, 'full', index = False)
writer_remaining.save()
writer_remaining.close() 
