...
# normal
df_cand['normal'] = df_cand['最高学历毕业院校'.decode('utf-8')].str.contains('师范'.decode('utf-8'))
...
# specialty 
list_specialty = list(df_cand['除文化课知识外我有艺术、体育、竞技等方面的爱好/特长：'.decode('utf-8')])
...
def ratio_balance_trigger(df_city_cand, df_sub_selected, dict_quota, dict_counter):
	...
	teacher_spec = list(df_sub_selected['除文化课知识外我有艺术、体育、竞技等方面的爱好/特长：'.decode('utf-8')])[0]
	...
	if teacher_spec == '是'.decode('utf-8'):
		...
		if dict_counter['city_spec_count'] > dict_quota['quota_spec']:
			df_city_cand = df_city_cand[df_city_cand['除文化课知识外我有艺术、体育、竞技等方面的爱好/特长：'.decode('utf-8')] != '是'.decode('utf-8')]
	...
	for index, row in df_city_school.iterrows():
		...
		if len(df_s) > 0:
			...
			if req_ppl > 0:
				...
				name_school = list(df_s['学校名称'.decode('utf-8')])[0].decode('utf-8')
				...
				if school_med_condition == '一般'.decode('utf-8'):
					df_city_cand = df_city_cand[pd.isnull(df_city_cand['12、我的过往病史为：'.decode('utf-8')])]
				## rule 5: requirement on province
				school_province = list(df_target_school['省'.decode('utf-8')])[0]
				df_city_cand = df_city_cand[(df_city_cand['我认为我在下列哪个地区能发挥更大的影响力，并说明理由：'.decode('utf-8')] == school_province)| (df_city_cand['我认为我在下列哪个地区能发挥更大的影响力，并说明理由：'.decode('utf-8')] == '都可以'.decode('utf-8'))]
				...
				for index, row in df_s.iterrows():
					if row['文理科'.decode('utf-8')] == '文科'.decode('utf-8'):
						...
						if len(df_sub_pool) > 0:
							...
							selected_teacher_name = list(df_sub_selected['全名'.decode('utf-8')])[0]
							df_s.loc[index, '老师'.decode('utf-8')] = selected_teacher_name
							df_cand = df_cand[df_cand['全名'.decode('utf-8')] != selected_teacher_name]
							df_city_cand = df_city_cand[df_city_cand['全名'.decode('utf-8')] != selected_teacher_name]
							df_city_cand, dict_counter = ratio_balance_trigger(df_city_cand, df_sub_selected, dict_quota, dict_counter)
					...
					if row['文理科'.decode('utf-8')] == '特长'.decode('utf-8'):
						df_sub_pool = df_city_cand[df_city_cand['除文化课知识外我有艺术、体育、竞技等方面的爱好/特长：'.decode('utf-8')] == '是'.decode('utf-8')] # potentially add another source of specialty
						if len(df_sub_pool) > 0:
							df_sub_selected = df_sub_pool.iloc[[0]]
							selected_teacher_name = list(df_sub_selected['全名'.decode('utf-8')])[0]
							df_s.loc[index, '老师'.decode('utf-8')] = selected_teacher_name
...
num_assigned = len(df_exp['老师'.decode('utf-8')].unique()) - 1
num_available = len(df_teacher)

if num_assigned != num_available:
	print('Warning: Not all teachers are assigned - {0} candidates available'.format(num_available - num_assigned))
	df_remain = df_teacher[~df_teacher['全名'.decode('utf-8')].isin(df_exp['老师'.decode('utf-8')].unique())]
	print(df_remain)